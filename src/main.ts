...
// 1. KEEP YOUR DATABASE NAME (As you requested)
const db = getFirestore(app, "autom8db"); 

// 2. SETUP FUNCTIONS (Connect to Jakarta Region)
...
async function generateTasksFromBrief(text: string): Promise<Task[]> {
  ...
  try {
    ...
    const aiTasks: Task[] = data.tasks.map((t, index) => ({
        ...
        name: t.name,
        type: t.type || 'Backend',
        // Smart Assignment: Find a real team member from your DB!
